...
if (__thisIsNewer) {
    ...
    function ajax(params){
        try {
            ...
            if (params.dataType.toLowerCase() == 'jsonp') {
                ...
                if (params.shard_data && params.query && !$w.isObject(params.query) && params.query.length > browser_url_limit) {
                    need_to_shard = true;
                ...
                } else if (params.query && $w.isObject(params.query)) {
                    query = $w.toStringAlt(params.query, '=', '&',true);
                ...
                url += (params.url.indexOf('?') != -1 ? "&" : "?") + "callback=" + func + (query || "");         
                ...
                if (params.query && $w.isObject(params.query)) {
                    params.query = $w.toStringAlt(params.query, '=', '&', true);
                }
    ...
    //        if (!variable) {
    //            var allkeyvalues = {},
    //            search = defer && $COMMIT.search || $l.search,
    //            hash =  defer && $COMMIT.hash || $l.hash,
    //            mapFunction = function(obj){var keyval = obj.split('=');return eval('({"'+keyval[0]+'":"'+keyval[1]+'"})');},
    //            sarray = search[0] == '?' && (search.substr(1)).split('&').map(mapFunction) || [],
    //            harray = hash[0] == '#' && (hash.substr(2)).split('@').map(mapFunction) || [];
    //            
    //            return sarray.concat(harray);
    //        
    //        }
    ...
    */
    ...
    function fillTemplate (htmlTemplate, objs) {
        try {
            ...
            htmlTemplate.isDomElement() && (htmlTemplate = htmlTemplate.toString());
            ...
            for (var i = 0, len = objs.length; i < len; i++) {
                ...
                for (var property in obj) {
                    ...
                    if (template.indexOf(expression) != -1 && obj.hasOwnProperty(property) && (objval = obj[property])) {
                ...
                // special run sytax
                template = __run_replace (/\$\{RUN\[(.+?)\]\}/, template, true, obj) || "";
                template = template.replace(/(\$\{[a-zA-Z$_]*\})/g, '');
                var tmp, rptmp;
                if ((tmp = /\$\{(.+?\|\|?.+?)\}/.exec(template)) && tmp[1]) {
                    tmp = tmp[1];
    ...
    _extend(String, 'toDateTime', function (options) {
     ...
     _extend(String, 'startsWith', function (str) {
         return str == this.slice(0, str.length);
     });
     _extend(String, 'endsWith', function (str) {
         return str == this.slice(-str.length);
     });
    ...
    //    if (this === $w) {
    //        var co = {};
    //        co.duplicate ? co.duplicate(this, true) : $w.duplicate(co, this, true);
    //        return co;
    //    }
